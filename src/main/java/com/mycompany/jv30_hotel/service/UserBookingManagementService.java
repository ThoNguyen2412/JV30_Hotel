...
//    add
    public void removeBookingForCancel(int bookingId) {
        BookingEntity booking = bookingRepository.findBookingReception(bookingId);
        List<BookingDetailEntity> details = booking.getBookingDetailEntities();
        Iterator<BookingDetailEntity> it = details.iterator();
        while (it.hasNext()) {
            BookingDetailEntity detail = it.next();
            bookingDetailRepo.delete(detail.getId());
        }
        CustomerInfoEntity customer = customerRepository.findCustomerByBookingId(bookingId);
